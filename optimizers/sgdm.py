...
class SGDM:
    ...
    def update(self, grads, states):
        ...
        updated_momentum = optree.tree_map(lambda m, g: self.momentum * m + (1 - m) * g, momentum, grads)

        corrected_momentum = optree.tree_map(lambda m: m / (1 - self.momentum), updated_momentum)

        updates = optree.tree_map(lambda lr, m: -lr * m, self.lr_groups, corrected_momentum)
