...
def parse_args() -> dotdict:
    ...
    )  # layer to extract mlp-post-non-lin features from, only if using real model
    parser.add_argument(
        "--layer_loc", type=str, default="residual"
    ...
    )  # How many batches to run the inner loop for before cutting out, 0 means run all
    parser.add_argument(
    ...
    parser.add_argument(
        "--center_activations", type=bool, default=False
    )  # Whether to center the activations before training autoencoder

    # interpret
    parser.add_argument("--n_feats_explain", type=int, default=10)  # number of features to explain
    parser.add_argument(
        "--activation_transform", type=str, default="feature_dict"
    )  # way of transforming neuron activations into features
