...
def read_all_layers(score_mode: str, layer_loc: str) -> None:
    ...
    # filter transforms
    transforms = ["tied_r0.5_l1a0.00086", "tied_r1.0_l1a0.00086", "tied_r2.0_l1a0.00086", "tied_r4.0_l1a0.00086", "tied_r8.0_l1a0.00086", "tied_r16.0_l1a0.00086", "tied_r32.0_l1a0.00086"]
    ...
    markers = ["o", "v", "^", "*", "x", "<", ">", "s", "p", "P", "h", "H", "+", "x", "X", "D", "d", "|", "_"]
    for i in layers:
        for j, transform in enumerate(transforms):
            plt.errorbar(i + 1 + (j * 0.07), means[i][j], yerr=cis[i][j], fmt="o", color=colors[j % len(colors)], elinewidth=1, capsize=0, markersize=8, marker=markers[j % len(markers)])
    ...
    for i, transform in enumerate(transforms):
        transform_names.append(f"{i+1}: sparse coding ratio = {transform.split('_')[1][1:]}")
    plt.legend(transform_names, loc="lower left")
    
    plt.axhline(y=0, linestyle="-", color="black", linewidth=1)
    plt.tight_layout()
    save_path = os.path.join(plots_folder, f"{layer_loc}_{score_mode}_ratios_means_and_cis.png")
