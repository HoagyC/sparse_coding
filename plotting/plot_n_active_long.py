...
def run_for_layer(args) -> None:
    ...
    if reload or not os.path.exists(os.path.join(PLOT_DATA_DIR, f"n_active_{tied}_l{layer}_{layer_loc}_long.pkl")):
        ...
        #     dicts_loc = f"tied_{layer_loc}_l{layer}_r{ratio}"
            print(dicts_loc)
            if not os.path.exists(os.path.join(LOAD_DIR, dicts_loc, "_80", "learned_dicts.pt")):
                continue
            all_dicts = torch.load(os.path.join(LOAD_DIR, dicts_loc, "_80", "learned_dicts.pt"))
        ...
        pickle.dump(layer_data, open(os.path.join(PLOT_DATA_DIR, f"n_active_untied_l{layer}_{layer_loc}.pkl"), "wb"))
        print(f"Finished layer {layer} {layer_loc}")
    
    else:
        layer_data = pickle.load(open(os.path.join(PLOT_DATA_DIR, f"n_active_untied_l{layer}_{layer_loc}.pkl"), "rb"))
    ...
    plt.savefig(os.path.join(PLOTS_DIR, f"active_plot_{tied}_l{layer}_{layer_loc}_long.png"))
...
if __name__ == "__main__":
    ...
    with mp.Pool(6) as pool:
        pool.map(run_for_layer, [(layer, layer_loc, devices[i], reload, tied) for i, layer in enumerate(layers)])
