...
def run_ablating_model_directions(
    ...
    **kwargs,
    ...
    if run_with_cache:
        return transformer.run_with_cache(tokens, fwd_hooks=other_hooks + [(tensor_name, intervention)], **kwargs)
    else:
        return transformer.run_with_hooks(tokens, fwd_hooks=other_hooks + [(tensor_name, intervention)], **kwargs)
...
def measure_concept_erasure(
    ...
    **kwargs,
    ...
    assert features_to_ablate.shape == (
        model.n_feats,
    ), f"to ablate shape {features_to_ablate.shape} is not {model.n_feats}"  # features_to_ablate is a binary vector of length n_feats indicating which features to ablate
    ...
    with torch.no_grad():
        transformer.run_with_hooks(tokens, fwd_hooks=[(read_t_name, read_tensor_hook)], **kwargs)
    y_true = activation_cache.reshape(B * L, -1)

    with torch.no_grad():
        run_ablating_model_directions(
            transformer,
            model,
            ablation_t_name,
            features_to_ablate,
            tokens,
            other_hooks=[(read_t_name, read_tensor_hook)],
            **kwargs,
        )
...
async def get_synthetic_dataset(
    explanation: str, sentences: List[str], tokenizer, str_len: int = 64
    ...
    for sentence in sentences:
        ...
        assert len(simulation.expected_activations) == len(
            tokens[0]
        ), f"Expected activations length {len(simulation.expected_activations)} does not match tokens length {len(tokens[0])}"
...
def process_scored_simulation(
    simulation: ScoredSimulation, tokenizer: HookedTransformer
) -> Tuple[List[List[str]], List[List[float]], List[List[int]]]:
