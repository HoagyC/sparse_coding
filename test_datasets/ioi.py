...
names = ["Jennifer", "Richard", "Catherine", "Laura", "Jake", "Parker", "Jeff", "Charlotte", "Michael", "Max"]
...
def generate_ioi_dataset(
    ...
    for i in range(n_abb_a):
        (name_a, name_b), location, object = np.random.choice(names, size=2, replace=False), np.random.choice(locations), np.random.choice(objects)
